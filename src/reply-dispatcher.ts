...
function stripInternalToolSyntaxLeak(text: string): string {
  const cleaned = text
    .split(/\r?\n/)
    .filter(
      (line) =>
        !/^\s*(sessions_send|sessions_spawn|subagents|message)\s*\([^)]*(sessionKey|agentId|target|accountId|label)\s*=.*\)\s*$/u.test(
          line.trim(),
        ),
    )
    .join("\n")
...
function stripRedundantLeadingMentions(text: string, hasMentionTargets: boolean): string {
  if (!hasMentionTargets) {
    return text;
  }
  return text
    .replace(/^(?:\s*<at\s+id=[^>]+><\/at>\s*)+/u, "")
...
export function createFeishuReplyDispatcher(params: CreateFeishuReplyDispatcherParams) {
  ...
  const collaborationRuntimeManaged = Boolean(params.collaborationTaskId);
  const renderMode = collaborationRuntimeManaged ? "raw" : (account.config?.renderMode ?? "auto");
  // Card streaming may miss thread affinity in topic contexts; use direct replies there.
  const streamingEnabled =
    !collaborationRuntimeManaged &&
    !threadReplyMode &&
