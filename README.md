...
推荐直接在群消息里声明模式。要稳定发布给别人用，进一步建议**显式声明参与者**，不要再把飞书每个 bot 账号看到的 `@` 当成唯一真相源。
...
```text
@首席大管家 #直答(Flink-SRE,Starrocks-SRE) 你俩各用一句话说下你们先看什么
```

```text
@首席大管家 #协作(Flink-SRE,Starrocks-SRE) 你俩讨论什么是灵魂，先各自判断，再互相补充，最后形成一句结论
```
...
- 协作模式是控制协议
- 用开放集去驱动控制协议，本身就不稳
- 飞书不同 bot 账号看到的 group mention 视角也不一定一致
